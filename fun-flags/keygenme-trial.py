...
key_part_static1_trial = "picoCTF{1n_7h3_|<3y_of_" # Length of 23
...
def enter_license():
    ...
    user_key = user_key.strip() #.strip() removes white spaces from the function
...
def check_key(key, username_trial):
    ...
    global key_full_template_trial # Our flag

    if len(key) != len(key_full_template_trial): # Our user-input should have the length of (32)
        print("length of your input is not equal to the lenth of the flag")
        print("the len of the flag is: ", len(key_full_template_trial), "while the len of your input is: ", len(key))
        return False
    else:
        ...
        if key[i] != hashlib.sha256(username_trial).hexdigest()[4]: # i=23
            return False
        else:
            i += 1

        if key[i] != hashlib.sha256(username_trial).hexdigest()[5]: # i=24
            return False
        else:
            i += 1

        if key[i] != hashlib.sha256(username_trial).hexdigest()[3]: # i=25
            return False
        else:
            i += 1

        if key[i] != hashlib.sha256(username_trial).hexdigest()[6]: # i=26
            return False
        else:
            i += 1

        if key[i] != hashlib.sha256(username_trial).hexdigest()[2]: # i=27
            return False
        else:
            i += 1

        if key[i] != hashlib.sha256(username_trial).hexdigest()[7]: # i=28
            return False
        else:
            i += 1

        if key[i] != hashlib.sha256(username_trial).hexdigest()[1]: # i=29
            return False
        else:
            i += 1

        if key[i] != hashlib.sha256(username_trial).hexdigest()[8]: # i=30
            return False
